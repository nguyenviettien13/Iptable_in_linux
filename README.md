...
target     prot opt source               destination
 ...
 ```
Khi bạn muốn chặn những message từ một máy trong mạng
 ````
sudo iptables -A INPUT -s 192.168.0.23 -j DROP
 ````
kết quả: 
 ```
tiennv000@td:~$ sudo iptables -L
Chain INPUT (policy ACCEPT)
target     prot opt source               destination
DROP       all  --  192.168.0.23         anywhere
 ```

Khi bạn muốn chặn tất cả mọi người trong mạng nhắn tin cho bạn 25(SMTP)
 ````
	tiennv000@td:~$ sudo iptables -A INPUT -s 192.168.0.0/24 -p tcp --destination-port 25
	* Kết quả *
	tiennv000@td:~$ sudo iptables -L
	Chain INPUT (policy ACCEPT)
	target     prot opt source               destination
 ...
 ````
Nhưng h bạn lại muốn nhận tin nhắn từ một nguời bạn tên D nào nào đó ở địa chỉ 192.168.0.26
 ```
	tiennv000@td:~$ sudo iptables -I INPUT -s 192.168.0.26 -j ACCEPT # tại sao ta phải dùng -I
	Nếu dùng -A thì rule này sẽ được áp dụng cuối cùng và gói tin từ bị DROP trước khi đến được ACCEPT.
	* Kết quả *
	tiennv000@td:~$ sudo iptables -L
	Chain INPUT (policy ACCEPT)
	target     prot opt source               destination         
	ACCEPT     all  --  192.168.0.26         anywhere            
	DROP       all  --  192.168.0.23         anywhere            
	           tcp  --  192.168.0.0/24       anywhere             tcp dpt:smtp
